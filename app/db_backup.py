...
def create_backup(_backup_file: str, _retries: int = 3, _retries_interval: int = 30):
    for _attempt in range(_retries):
        ...
        dump_command = [
            'pg_dump',
            '--dbname=postgresql://{}:{}@{}:{}/{}'.format(config.db_main_user,
                                                          config.db_main_password,
                                                          config.db_main_host,
                                                          config.db_main_port,
                                                          config.db_main_name),
            '-F', 'c',
...
def restore_backup(_conn, _backup_file: str, do_db_init=True, _retries: int = 3, _retries_interval: int = 30):
